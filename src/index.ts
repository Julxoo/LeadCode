...
function buildSetupSteps(projectPath: string, isFr: boolean): string[] {
  if (isFr) {
    return [
      ...
      "- `engines`, `packageManager`, `workspaces` (si présents) : identifie le contexte d'exécution.",
      "",
  ...
  return [
    ...
    "- `engines`, `packageManager`, `workspaces` (if present): identify the execution context.",
    "",
  ...
  ({ projectPath, language }) => {
    ...
    const preliminary = isFr
      ? [
          ...
          "",
          "Règles de filtrage :",
          "- Si la tech/sujet est toujours dans les dépendances du projet → garde la décision.",
      ...
      : [
          ...
          "",
          "Filtering rules:",
          "- If the tech/topic is still in the project dependencies → keep the decision.",
